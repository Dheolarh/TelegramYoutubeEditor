...
const getConnectedChannel = async (telegramChatId: string) => {
  try {
    ...
    console.warn('DB error in getConnectedChannel:', err.message);
    return null;
  }
};
...
bot.action('cmd_disconnect', async (ctx) => {
  ...
  if (!connData) {
    return ctx.reply('<b>No YouTube channel connected to disconnect.</b>', { parse_mode: 'HTML' });
  }
...
const handleModerationCommand = async (ctx: any) => {
  ...
  if (!connData) {
    return ctx.reply('<b>Connect your YouTube channel via /start first.</b>', { parse_mode: 'HTML' });
  }
  ...
  const text = isEnabled
    ? `<b>AUTOMATED COMMENT MODERATION ENABLED</b> (Active)\n\n` +
    ...
    : `<b>AUTOMATED COMMENT MODERATION DISABLED</b> (Off)\n\n` +
...
bot.action('cmd_mod_scan', async (ctx: any) => {
  await ctx.answerCbQuery();
  const telegramChatId = ctx.from.id.toString();
  await ctx.reply('<b>Scanning your YouTube comments for profanity & spam...</b>', { parse_mode: 'HTML' });
...
const handleAnalyticsCommand = async (ctx: any, days: number = 30, isEdit: boolean = false) => {
  ...
  if (!connData) {
    const msg = '<b>No YouTube channel connected yet.</b>\n\nConnect via /start.';
    return isEdit
  ...
  try {
    ...
    const keyboard = Markup.inlineKeyboard([
      [
        ...
        Markup.button.callback('Last 90 Days', 'ana_90d'),
      ],
      [
...
const renderVideoList = async (ctx: any, page: number = 1, isEdit: boolean = false) => {
  ...
  try {
    ...
    let text = `<b>Recent YouTube Videos (Page ${currentPage}/${totalPages}):</b>\n\n`;
    ...
    const navRow: any[] = [];
    if (currentPage > 1) {
      navRow.push(Markup.button.callback('Prev', `vpage_${currentPage - 1}`));
    }
    if (currentPage < totalPages) {
      navRow.push(Markup.button.callback('Next', `vpage_${currentPage + 1}`));
    }
...
bot.command('search', async (ctx: any) => {
  const telegramChatId = ctx.from.id.toString();
  const connData = await getConnectedChannel(telegramChatId);
  if (!connData) return ctx.reply('<b>Connect via /start first.</b>', { parse_mode: 'HTML' });
...
bot.action(/^vid_(.+)$/, async (ctx) => {
  ...
  const text =
    ...
    `<b>Tags:</b> ${video.tags.slice(0, 5).join(', ') || 'No tags'}\n\n` +
    `<b>Choose an action:</b>`;

  const keyboard = Markup.inlineKeyboard([
    [Markup.button.callback('Edit Title', `act_title_${video.id}`), Markup.button.callback('Edit Description', `act_desc_${video.id}`)],
    [Markup.button.callback('Edit Tags', `act_tags_${video.id}`), Markup.button.callback('Update Thumbnail', `act_thumb_${video.id}`)],
...
bot.action(/^act_title_(.+)$/, async (ctx) => {
  ...
  await ctx.reply(`<b>New title for:</b> "${video.title}"\n\nSend your new title:`, { parse_mode: 'HTML' });
});
...
bot.action(/^act_tags_(.+)$/, async (ctx) => {
  ...
  await ctx.reply(`<b>New tags for:</b> "${video.title}"\n\nSend comma-separated tags:`, { parse_mode: 'HTML' });
});
...
bot.action(/^act_thumb_(.+)$/, async (ctx) => {
  ...
  await ctx.reply(`<b>Upload thumbnail for:</b> "${video.title}"\n\nSend a photo in this chat:`, { parse_mode: 'HTML' });
});
  ...
  try {
    ...
    for (const c of comments) {
      ...
      await ctx.reply(
        ...
        {
          parse_mode: 'HTML',
          ...Markup.inlineKeyboard([[Markup.button.callback(`Reply to ${authorShort}`, replyCallback)]]),
        }
      );
    }
  } catch (err: any) {
...
bot.action(/^rc_(.+)$/, async (ctx) => {
  ...
  await ctx.reply('<b>Type your reply to this comment:</b>', { parse_mode: 'HTML' });
});
...
bot.action(/^act_ai_(.+)$/, async (ctx) => {
  ...
  await ctx.reply(text, {
    parse_mode: 'HTML',
    ...Markup.inlineKeyboard([
      [Markup.button.callback('1. Generate New Thumbnail', `ais_thumb_${video.id}`)],
      [Markup.button.callback('2. Generate New Title', `ais_title_${video.id}`)],
      [Markup.button.callback('3. Generate New Description', `ais_desc_${video.id}`)],
      [Markup.button.callback('4. Generate New Keywords/Tags', `ais_tags_${video.id}`)],
      [Markup.button.callback('5. Create Pinned Comment', `ais_pincmt_${video.id}`)],
      [Markup.button.callback('6. Back to Video', `vid_${video.id}`)],
    ]),
  });
});
...
bot.action(/^ais_thumb_(.+)$/, async (ctx) => {
  ...
  try {
    ...
    const caption = `<b>AI Generated Thumbnail for:</b>\n"${video.title}"\n\nChoose an action below:`;
    const keyboard = Markup.inlineKeyboard([
      [
      ...
      [
        Markup.button.callback('Custom Prompt / Ref', `aie_thumb_${video.id}`),
...
bot.action(/^aie_thumb_(.+)$/, async (ctx) => {
  ...
  const promptText =
    `<b>Custom AI Thumbnail Guidance for:</b>\n"${video.title}"\n\n` +
    ...
    await ctx.reply(text, {
      ...
      ...Markup.inlineKeyboard([
        [
          ...
          Markup.button.callback('Back to Video', `vid_${video.id}`),
        ],
      ]),
    });
  } catch (err: any) {
...
bot.on('photo', async (ctx: any) => {
  ...
  if (state.action === 'AWAITING_THUMB_CUSTOM') {
    ...
    try {
      ...
      const caption = `<b>Custom AI Reference Thumbnail for:</b>\n"${video.title}"\n\nChoose an action below:`;
      const keyboard = Markup.inlineKeyboard([
        [
          Markup.button.callback('Apply', `aia_thumb_${video.id}`),
          Markup.button.callback('Regenerate', `ais_thumb_${video.id}`),
...
bot.on('text', async (ctx: any) => {
  ...
  try {
    if (state.action === 'AWAITING_TITLE') {
      ...
      const caption = `<b>Custom AI Thumbnail for:</b>\n"${video.title}"\n<i>Instructions: "${text}"</i>\n\nChoose an action below:`;
      const keyboard = Markup.inlineKeyboard([
        [
          Markup.button.callback('Apply', `aia_thumb_${video.id}`),
          Markup.button.callback('Regenerate', `ais_thumb_${video.id}`),
...
export const initTelegramBot = async (publicUrl?: string, retries = 5): Promise<void> => {
  ...
  try {
    ...
    console.warn('deleteWebhook info:', err.message);
  }
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.warn(`Bot launch attempt ${attempt}/${retries}: ${err.message}`);
      if (attempt < retries) {
